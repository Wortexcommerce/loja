<?php
class ImagensRedimensiona{

	public function RedimensionaFotoProduto($imagem, $tamanho, $tipo){	
		
			$oImg = new m2brimagem();
        	$oImg->carrega('../../../'.PASTA_UPLOAD_IMAGENS_GRANDE.$imagem);
		
		
			$imnfo = getimagesize('../../../'.PASTA_UPLOAD_IMAGENS_GRANDE.$imagem);
			...
			if($img_w>$img_h){
				...
				if($img_w>$tamanho || $img_w==$tamanho){					
			...
			//FOTO EM PÉ	
				...
				if($img_h>$tamanho || $img_h==$tamanho){					
					$oImg->redimensiona($new_largura,$new_altura);								
				} 
			...
			if($tipo=='thumb'){
				$retorno = $oImg->grava('../../../'.PASTA_UPLOAD_IMAGENS_THUMBS.$imagem,100);
			}
			
			if($tipo=='grande'){
				$retorno = $oImg->grava('../../../'.PASTA_UPLOAD_IMAGENS_GRANDE.$imagem,100);
			} 
...
//CLASSE DE DIMENSIONAMENTO DE IMAGENS
class m2brimagem {
